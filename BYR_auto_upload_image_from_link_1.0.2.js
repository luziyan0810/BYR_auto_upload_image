...
.pic_info_auto_field_new .pic_info_auto_field_val {
    ...
    color: red;
        ...
        $('#pic_info_cover_check').click(function () {
            ...
            GM_request(`${byr}?ModPagespeed=off`, 'blob').then(blob => {
                ...
                function notSame() {
                    //log('BYR服务器上存在同名文件，但是内容不同（或者无法确定是否相同），你可以尝试改名上传，或者 ', -1, 'cover');
                    //$('<a class="pic_info_cover_link">尝试使用该文件</a>').attr('href', byr).appendTo('#pic_info_cover_status');
                    log('BYR服务器上存在同名文件，但图片内容不同（软件自动检测），将自动改名重传', -1, 'cover');
                    $('#pic_info_cover_upload').click()
                }
            }).catch(() => {
                log('BYR服务器上没有同名文件，请尝试上传', -1, 'cover');
                //$('#pic_info_cover_upload').show();
                $('#pic_info_cover_upload').click()
            });
        });
        ...
        function renderPreview() {
            ...
            if (coverReady) preview.append(`<img src="${cover}" style="max-width: 90%;">`);
            for (let key in descr) {
                $(`<p>【${key}】</p>`).appendTo(preview);
                $(`<p></p>`).html(descr[key].map(escapeHtml).join('<br>')).appendTo(preview);
            }
            let meta = $(`<p class="byrbt_pic_info" data-version="${GM_scriptVersion}" style="font-size: 0.8em; opacity: 0.5; margin-bottom: 0;">以上内容由 BYRBT pic Info 自动生成</p>`);
    ...
    function AutoFields(renderPreview) {
        ...
        return {
            init(selector) {
                wrapper = $(selector);
                wrapper.html('<br>字段填写建议，不一定准确，请注意检查，点击自动填写（仅限发布页面，<span class="pic_info_auto_field_new"><span class="pic_info_auto_field_val">高亮</span></span>表示与目前填写的内容不同），Ctrl+点击复制到剪贴板：<span id="pic_info_auto_fields_status"></span><div id="pic_info_auto_fields"></div><button type="button" id="pic_info_row_toggle"></button>');
                wrapper.hide();
            },
            row() {
                return rowFlag && row;
