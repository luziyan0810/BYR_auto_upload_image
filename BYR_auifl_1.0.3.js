...
.pic_info_auto_field_new .pic_info_auto_field_val {
    ...
    color: red;
        ...
        $('#pic_info_cover_filename').change(function () {
            ...
            //filename = filename.substr(0,lastIndexOf('.') + 1);
            filename = filename.replace(/[^A-Za-z0-9._]/g, '');
            filename = filename.replace(/0x/ig, '0plusx');
            filename = filename.replace(/title/ig, 'titie');
            filename = filename.replace(/[.]/ig, '_')
            filename = filename.replace(/_png/ig, '.png')
            filename = filename.replace(/_jpg/ig, '.jpg')
            filename = filename.replace(/_jpeg/ig, '.jpeg')
            filename = filename.replace(/_bmp/ig, '.bmp')
            filename = filename.replace(/_gif/ig, '.gif')
            input.val(filename);
